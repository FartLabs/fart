 ...
 */
export const compileFartToTs = async (request: Request): Promise<Response | null> => {
  const url = new URL(request.url);

  // Match `/deno.cli/` or `/ts/` URL routing.
  if (url.pathname.startsWith("/deno.cli/") || url.pathname.startsWith("/ts/")) {
    const { sourceUrl, implUrl } = parseFartUrl(url);

    // Parse the endpoint suffix to fetch the raw source code
    const rawPath = sourceUrl.replace(/^\/(?:deno\.cli|ts)\//, "");
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch Fart source code: ${response.statusText}`);
      }
